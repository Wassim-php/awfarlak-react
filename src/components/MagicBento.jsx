...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = (event) => {
      if (!clickEffect && !onCardClick) return;
...
const BentoCardGrid = ({ children, gridRef, className, style }) => (
  <div className={`card-grid bento-section magic-bento ${className || ""}`} ref={gridRef} style={style}>
    {children}
  </div>
);
...
const MagicBento = ({
  ...
  return (
    <>
      ...
      <BentoCardGrid
        gridRef={gridRef}
        className={layout === "simple" ? "magic-bento--simple" : ""}
        style={gridStyle}
      >
        {items.map((card, index) => {
          const isCustom = Boolean(card.content);
          const isClickable = typeof card.onClick === "function";
          const baseClassName = `magic-bento-card ${textAutoHide ? "magic-bento-card--text-autohide" : ""} ${enableBorderGlow ? "magic-bento-card--border-glow" : ""} ${isCustom ? "magic-bento-card--custom" : ""} ${isClickable ? "magic-bento-card--clickable" : ""}`;
          const cardProps = {
          ...
          const content = card.content ? (
            <div className="magic-bento-card__custom">{card.content}</div>
          ) : (
            <>
